...
function executePlaylist(client, id) {
    return new Promise((resolve, reject) => {
       ...
       */
        ...
        let pager = new kaltura.objects.FilterPager();
        kaltura.services.playlist
            .execute(id, detailed, playlistContext, filter, pager)
            .execute(client)
            .then(response => {
                resolve(response);
            })
...
router.get("/", async function(req, res, next) {
    ...
    if (ks && playlistId) {
        ...
        try {
            ...
            if (
                entryList.totalCount === 0 ||
                !entryList.objects.every(
                    entry => entry.status.toString() === kaltura.enums.EntryStatus.READY
                )
            ) {
